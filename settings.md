...
- [ ] 各プロジェクトの `{project}-managers` Team のリーダーを **Team Maintainer** に指定
- [ ] 各プロジェクトの `{project}` Team のリーダーを **Team Maintainer** に指定
...
## 5. Organization Rulesets の設定（優先度：高）
...
5. **依頼チャンネルの整備**: Slack `#github-admin` チャンネルを作成し、IT 部への依頼フローを周知
