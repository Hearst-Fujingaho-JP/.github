...
- [ ] Repository creation を「Private のみ許可」に変更（現状: 全メンバーが Public/Private 両方作成可能。Public は Owner のみに制限）
...
- [ ] Require status checks to pass: **有効**（CI がある場合）
  - [ ] Require branches to be up to date before merging: **有効**（リポジトリ単位での緩和を許容。緩和する場合はリポジトリの README 等に理由を明記）
